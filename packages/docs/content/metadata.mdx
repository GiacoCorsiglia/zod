...
<Callout>
**Important** —  If you register metadata on an a schema and include an `id` property, Zod will throw an error if multiple schemas share the same `id` value. Make sure all registered `id` values are unique per registry. This also applies to the global registry.
</Callout>
